...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .sku-table table { min-width: 1460px; }
    .sku-trend-table table { min-width: 1180px; }
    .channel-breakdown { display: grid; gap: 4px; min-width: 150px; }
    .channel-line { display: grid; grid-template-columns: 62px minmax(54px, auto) auto; align-items: baseline; gap: 6px; line-height: 1.25; }
    .channel-name { color: var(--ink); font-weight: 800; }
    .channel-value { color: var(--muted); font-weight: 700; text-align: right; white-space: nowrap; }
    .channel-delta { font-weight: 800; white-space: nowrap; }
    .channel-delta.up { color: var(--green); }
    .channel-delta.down { color: var(--red); }
    .channel-delta.flat { color: var(--muted); }
    ...
    .summary-total-grid { display: grid; grid-template-columns: 1.4fr repeat(4, minmax(0,1fr)); gap: 12px; align-items: center; }
...
<body>
  <div class="page">
    ...
    <section class="filters">
      ...
      <div class="filter-card"><label for="compareMode">So sánh</label><select id="compareMode"><option value="previousMonth">Cùng kỳ tháng trước</option><option value="previousPeriod">Kỳ liền trước</option><option value="previousYear">Cùng kỳ năm trước</option><option value="custom">Tùy chọn</option><option value="none">Không so sánh</option></select><div class="filter-hint">Chọn kỳ để tính % tăng/giảm</div></div>
      <div class="filter-card"><label for="compareFromDate">So sánh từ ngày</label><input id="compareFromDate" type="date" /></div>
      <div class="filter-card"><label for="compareToDate">So sánh đến ngày</label><input id="compareToDate" type="date" /></div>
    </section>
  ...
  <script>
    ...
    function setup() {
      ...
      document.getElementById("fromDate").value = state.from; document.getElementById("toDate").value = state.to; document.getElementById("compareMode").value = state.compareMode;
      ...
      document.getElementById("compareMode").addEventListener("change", e => { state.compareMode = e.target.value; const comparison = resolveComparison(); if (state.compareMode === "custom") { state.compareFrom = comparison.from || state.compareFrom || state.from; state.compareTo = comparison.to || state.compareTo || state.to; } render(); });
      document.getElementById("compareFromDate").addEventListener("change", e => { state.compareFrom = e.target.value; if (state.compareMode !== "custom") state.compareMode = "custom"; render(); });
      document.getElementById("compareToDate").addEventListener("change", e => { state.compareTo = e.target.value; if (state.compareMode !== "custom") state.compareMode = "custom"; render(); });
    ...
    function channelDeltaHtml(value) { const cls = Math.abs(value) < .05 ? "flat" : (value > 0 ? "up" : "down"); const arrow = Math.abs(value) < .05 ? "•" : (value > 0 ? "↑" : "↓"); return `<span class="channel-delta ${cls}">${arrow} ${formatPercent(Math.abs(value))}</span>`; }
    function channelBreakdownHtml(row, prev = {}) { const current = row.channelRevenue || {}, previous = prev.channelRevenue || {}; return `<div class="channel-breakdown">${CHANNEL_BREAKDOWN.map(channel => { const currentValue = Number(current[channel.key] || 0), previousValue = Number(previous[channel.key] || 0); return `<div class="channel-line"><span class="channel-name">${channel.label}</span><span class="channel-value">${formatCurrency(currentValue)}</span>${channelDeltaHtml(pctDelta(currentValue, previousValue))}</div>`; }).join("")}</div>`; }
    ...
    function resolveComparison() {
      if (state.compareMode === "none") return { enabled:false, label:"Không so sánh", from:"", to:"" };
      if (state.compareMode === "custom") {
        const from = state.compareFrom || state.from, to = state.compareTo || state.to;
        return { enabled:!!from && !!to, label:"So sánh tùy chọn", from, to };
      }
      if (state.compareMode === "previousPeriod") {
        const days = Math.max(dayDiff(state.from, state.to), 1), to = addDays(state.from, -1), from = addDays(to, 1 - days);
        return { enabled:true, label:"So sánh kỳ liền trước", from, to };
      }
      if (state.compareMode === "previousYear") return { enabled:true, label:"So sánh cùng kỳ năm trước", from:shiftMonthClamped(state.from, -12), to:shiftMonthClamped(state.to, -12) };
      return { enabled:true, label:"So sánh cùng kỳ tháng trước", from:shiftMonthClamped(state.from, -1), to:shiftMonthClamped(state.to, -1) };
    }
    function syncComparisonInputs(comparison) {
      const mode = document.getElementById("compareMode"), from = document.getElementById("compareFromDate"), to = document.getElementById("compareToDate");
      mode.value = state.compareMode;
      from.value = comparison.from || "";
      to.value = comparison.to || "";
      const editable = state.compareMode === "custom";
      from.disabled = !editable;
      to.disabled = !editable;
    }
    function isCanceled(item) { return String(item.x).toLowerCase() === "yes"; }
    function cancelStats(records) { const orders = new Set(), canceledOrders = new Set(); for (const item of records) { const order = item.o || `${item.s}|${item.d}|${item.c}`; orders.add(order); if (isCanceled(item)) canceledOrders.add(order); } return { orderCount: orders.size, canceledCount: canceledOrders.size, cancelRate: orders.size ? canceledOrders.size / orders.size * 100 : 0 }; }
    function summarize(records) { let revenue = 0, volume = 0; for (const item of records) { revenue += item.r; volume += item.q; } const cancel = cancelStats(records); return { revenue, volume, asp: volume ? revenue / volume : 0, cancelRate: cancel.cancelRate }; }
    function setMetric(valueId, deltaId, current, previous, formatter, inverse, comparison) { document.getElementById(valueId).textContent = formatter(current); const el = document.getElementById(deltaId); if (!comparison.enabled) { el.className = "delta flat"; el.textContent = "Không so sánh"; return; } const delta = pctDelta(current, previous); el.className = `delta ${Math.abs(delta) < .05 ? "flat" : (delta > 0 ? (inverse ? "down" : "up") : (inverse ? "up" : "down"))}`; el.innerHTML = `${Math.abs(delta) < .05 ? "•" : (delta > 0 ? "↑" : "↓")} ${formatPercent(Math.abs(delta))} vs ${periodLabel(comparison.from, comparison.to)}`; }
    function attachCancelRate(rows) { return rows.map(row => ({...row, cancelRate: row.orders?.size ? row.canceledOrders.size / row.orders.size * 100 : 0})); }
    function aggregateBy(records, keyField) { const map = new Map(); for (const item of records) { const key = item[keyField]; const row = map.get(key) || { key, revenue: 0, volume: 0, count: 0, orders: new Set(), canceledOrders: new Set(), channelRevenue: { shopee: 0, tiktokshop: 0, web: 0 } }; const order = item.o || `${item.s}|${item.d}|${item.c}`; row.revenue += item.r; row.volume += item.q; row.count += 1; row.orders.add(order); if (isCanceled(item)) row.canceledOrders.add(order); if (row.channelRevenue[item.c] !== undefined) row.channelRevenue[item.c] += item.r; map.set(key, row); } return attachCancelRate(Array.from(map.values())); }
    function aggregateSku(records) { const map = new Map(); for (const item of records) { const key = item.s; const row = map.get(key) || { sku: key, variant: item.v, product: item.p, group: item.g, keySummer: item.ks, classify: item.cl, image: item.i, url: item.u, revenue: 0, volume: 0, count: 0, orders: new Set(), canceledOrders: new Set(), channelRevenue: { shopee: 0, tiktokshop: 0, web: 0 } }; const order = item.o || `${item.s}|${item.d}|${item.c}`; row.revenue += item.r; row.volume += item.q; row.count += 1; row.orders.add(order); if (isCanceled(item)) row.canceledOrders.add(order); if (row.channelRevenue[item.c] !== undefined) row.channelRevenue[item.c] += item.r; if (!row.product || row.product.length < item.p.length) row.product = item.p; if (!row.image && item.i) row.image = item.i; if (!row.url && item.u) row.url = item.u; map.set(key, row); } return attachCancelRate(Array.from(map.values())); }
    ...
    function render() {
      ...
      syncComparisonInputs(comparison);
      const previous = comparison.enabled ? filterRecords(REPORT_DATA.records, comparison.from, comparison.to) : [];
      document.getElementById("comparisonNote").innerHTML = `<strong>Kỳ đang xem:</strong> ${periodLabel(state.from, state.to)} &nbsp;•&nbsp; <strong>${comparison.label}:</strong> ${comparison.enabled ? periodLabel(comparison.from, comparison.to) : "Không tính % tăng/giảm"}`;
      const s = summarize(current), p = summarize(previous);
      setMetric("metricRevenue","metricRevenueDelta",s.revenue,p.revenue,formatCurrency,false,comparison);
      setMetric("metricVolume","metricVolumeDelta",s.volume,p.volume,formatUnits,false,comparison);
      setMetric("metricAsp","metricAspDelta",s.asp,p.asp,formatCurrency,false,comparison);
      setMetric("metricCancel","metricCancelDelta",s.cancelRate,p.cancelRate,formatPercent,true,comparison);
    ...
    function renderSummaryTable(rows, prevMap, type) {
      if (!rows.length) return `<div class="empty">Không có dữ liệu.</div>`;
      const sorted = sortSummaryRows(rows, type), totalRevenue = rows.reduce((s,r) => s+r.revenue,0), totalVolume = rows.reduce((s,r) => s+r.volume,0), totalOrders = new Set(), totalCanceledOrders = new Set();
      rows.forEach(row => { row.orders?.forEach(order => totalOrders.add(order)); row.canceledOrders?.forEach(order => totalCanceledOrders.add(order)); });
      const titleMap = { channel:"Kênh bán hàng", group:"Group", keySummer:"Priority", classify:"Classify" };
      const visible = type === "group" ? sorted : sorted.slice(0, 12);
      const sort = state.summarySort[type], showChannels = type !== "channel";
      const arrow = key => sort.key === key ? (sort.dir === "desc" ? "▼" : "▲") : "↕";
      const body = visible.map((row, idx) => {
        const prev = prevMap.get(row.key) || {revenue:0, volume:0, cancelRate:0, channelRevenue:{}};
        const asp = row.volume ? row.revenue / row.volume : 0, prevAsp = prev.volume ? prev.revenue / prev.volume : 0, share = totalRevenue ? row.revenue / totalRevenue * 100 : 0;
        return `<tr><td class="rank">${idx+1}.</td><td>${escapeHtml(row.key)}<div><span class="subtle">${formatPercent(share)} DT</span></div></td><td>${formatCurrency(row.revenue)}<div>${deltaHtml(pctDelta(row.revenue, prev.revenue))}</div></td><td>${formatUnits(row.volume)}<div>${deltaHtml(pctDelta(row.volume, prev.volume))}</div></td><td>${formatCurrency(asp)}<div>${deltaHtml(pctDelta(asp, prevAsp))}</div></td><td>${formatPercent(row.cancelRate)}<div>${deltaHtml(pctDelta(row.cancelRate, prev.cancelRate), true)}</div></td>${showChannels ? `<td>${channelBreakdownHtml(row, prev)}</td>` : ""}</tr>`;
      }).join("");
      const channelHead = showChannels ? `<th>Kênh</th>` : "";
      return `<div class="table-scroll summary-table"><table><thead><tr><th></th><th><button class="sort-btn" data-summary-sort="${type}" data-sort-key="label">${titleMap[type]} ${arrow("label")}</button></th><th><button class="sort-btn" data-summary-sort="${type}" data-sort-key="revenue">DT ${arrow("revenue")}</button></th><th><button class="sort-btn" data-summary-sort="${type}" data-sort-key="volume">Volume ${arrow("volume")}</button></th><th><button class="sort-btn" data-summary-sort="${type}" data-sort-key="asp">ASP ${arrow("asp")}</button></th><th><button class="sort-btn" data-summary-sort="${type}" data-sort-key="cancelRate">% Hủy ${arrow("cancelRate")}</button></th>${channelHead}</tr></thead><tbody>${body}</tbody></table></div><div class="summary-total"><div class="summary-total-grid"><div class="summary-stat-value">Tổng cộng</div><div class="summary-stat"><div class="summary-stat-label">DT</div><div class="summary-stat-value">${formatCurrency(totalRevenue)}</div></div><div class="summary-stat"><div class="summary-stat-label">Volume</div><div class="summary-stat-value">${formatUnits(totalVolume)}</div></div><div class="summary-stat"><div class="summary-stat-label">ASP</div><div class="summary-stat-value">${formatCurrency(totalVolume ? totalRevenue / totalVolume : 0)}</div></div><div class="summary-stat"><div class="summary-stat-label">% Hủy</div><div class="summary-stat-value">${formatPercent(totalOrders.size ? totalCanceledOrders.size / totalOrders.size * 100 : 0)}</div></div></div></div>`;
    }
    ...
    function renderSkuTableMarkup(rows, prevMap) {
      const sorted = sortSkuRows(rows).slice(0, 100), sort = state.skuSort, arrow = key => sort.key === key ? (sort.dir === "desc" ? "▼" : "▲") : "↕";
      const body = sorted.map((row, idx) => {
        const prev = prevMap.get(row.sku) || {revenue:0, volume:0, cancelRate:0, channelRevenue:{}};
        const asp = row.volume ? row.revenue / row.volume : 0, prevAsp = prev.volume ? prev.revenue / prev.volume : 0;
        const name = row.url ? `<a class="product-link" href="${escapeAttr(row.url)}" target="_blank" rel="noopener">${escapeHtml(row.product)}</a>` : escapeHtml(row.product);
        const img = imageSrc(row);
        return `<tr><td class="rank">${idx+1}.</td><td>${img ? `<img class="sku-thumb" src="${escapeAttr(img)}" alt="${escapeAttr(row.product)}" loading="lazy" />` : ""}</td><td><span class="sku-code">${escapeHtml(row.sku)}</span><div class="subtle">${escapeHtml(row.variant || "-")}</div></td><td class="raw-product">${name}<div class="subtle">${escapeHtml(row.keySummer)} · ${escapeHtml(row.classify)}</div></td><td>${escapeHtml(row.group)}</td><td>${formatCurrency(row.revenue)}<div>${deltaHtml(pctDelta(row.revenue, prev.revenue))}</div></td><td>${formatUnits(row.volume)}<div>${deltaHtml(pctDelta(row.volume, prev.volume))}</div></td><td>${formatCurrency(asp)}<div>${deltaHtml(pctDelta(asp, prevAsp))}</div></td><td>${formatPercent(row.cancelRate)}<div>${deltaHtml(pctDelta(row.cancelRate, prev.cancelRate), true)}</div></td><td>${channelBreakdownHtml(row, prev)}</td></tr>`;
      }).join("");
      return `<div class="table-scroll sku-table"><table><thead><tr><th></th><th>Ảnh</th><th><button class="sort-btn" data-sku-sort data-sort-key="barcode">Barcode ${arrow("barcode")}</button></th><th><button class="sort-btn" data-sku-sort data-sort-key="product">Tên sản phẩm ${arrow("product")}</button></th><th><button class="sort-btn" data-sku-sort data-sort-key="group">Group ${arrow("group")}</button></th><th><button class="sort-btn" data-sku-sort data-sort-key="revenue">DT ${arrow("revenue")}</button></th><th><button class="sort-btn" data-sku-sort data-sort-key="volume">Volume ${arrow("volume")}</button></th><th><button class="sort-btn" data-sku-sort data-sort-key="asp">ASP ${arrow("asp")}</button></th><th><button class="sort-btn" data-sku-sort data-sort-key="cancelRate">% Hủy ${arrow("cancelRate")}</button></th><th>Kênh</th></tr></thead><tbody>${body}</tbody></table></div>`;
    }
    ...
    function renderSkuTrendTable(rows, prevMap, direction) {
      const trendRows = getSkuTrendRows(rows, prevMap, direction);
      if (!trendRows.length) return `<div class="empty">Không có dữ liệu.</div>`;
      const body = trendRows.map((row, idx) => {
        const prev = prevMap.get(row.sku) || {revenue:0, volume:0, cancelRate:0, channelRevenue:{}};
        const img = imageSrc(row);
        return `<tr><td class="rank">${idx+1}.</td><td>${img ? `<img class="sku-thumb" src="${escapeAttr(img)}" alt="${escapeAttr(row.product)}" loading="lazy" />` : ""}</td><td><span class="sku-code">${escapeHtml(row.sku)}</span><div class="subtle">${escapeHtml(row.product)}</div></td><td>${escapeHtml(row.group)}</td><td>${formatCurrency(row.revenue)}</td><td>${deltaHtml(row.deltaRevenue)}</td><td>${formatPercent(row.cancelRate)}<div>${deltaHtml(pctDelta(row.cancelRate, prev.cancelRate), true)}</div></td><td>${channelBreakdownHtml(row, prev)}</td></tr>`;
      }).join("");
      return `<div class="table-scroll sku-trend-table"><table><thead><tr><th></th><th>Ảnh</th><th>SKU</th><th>Group</th><th>DT</th><th>% Δ</th><th>% Hủy</th><th>Kênh</th></tr></thead><tbody>${body}</tbody></table></div>`;
    }

...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .sku-table table { min-width: 1360px; }
    .sku-trend-table table { min-width: 1080px; }
...
<body>
  ...
  <script>
    ...
    function renderSummaryTable(rows, prevMap, type) {
      ...
      const sorted = sortSummaryRows(rows, type), totalRevenue = rows.reduce((s,r) => s+r.revenue,0), totalVolume = rows.reduce((s,r) => s+r.volume,0);
      ...
      const body = visible.map((row, idx) => {
        const prev = prevMap.get(row.key) || {revenue:0, volume:0, channelRevenue:{}};
    ...
    function renderSkuTableMarkup(rows, prevMap) {
      ...
      const body = sorted.map((row, idx) => {
        const prev = prevMap.get(row.sku) || {revenue:0, volume:0, channelRevenue:{}};
        ...
        return `<tr><td class="rank">${idx+1}.</td><td>${img ? `<img class="sku-thumb" src="${escapeAttr(img)}" alt="${escapeAttr(row.product)}" loading="lazy" />` : ""}</td><td><span class="sku-code">${escapeHtml(row.sku)}</span><div class="subtle">${escapeHtml(row.variant || "-")}</div></td><td class="raw-product">${name}<div class="subtle">${escapeHtml(row.keySummer)} · ${escapeHtml(row.classify)}</div></td><td>${escapeHtml(row.group)}</td><td>${formatCurrency(row.revenue)}<div>${deltaHtml(pctDelta(row.revenue, prev.revenue))}</div></td><td>${formatUnits(row.volume)}<div>${deltaHtml(pctDelta(row.volume, prev.volume))}</div></td><td>${formatCurrency(asp)}<div>${deltaHtml(pctDelta(asp, prevAsp))}</div></td><td>${channelBreakdownHtml(row, prev)}</td></tr>`;
      }).join("");
      return `<div class="table-scroll sku-table"><table><thead><tr><th></th><th>Ảnh</th><th><button class="sort-btn" data-sku-sort data-sort-key="barcode">Barcode ${arrow("barcode")}</button></th><th><button class="sort-btn" data-sku-sort data-sort-key="product">Tên sản phẩm ${arrow("product")}</button></th><th><button class="sort-btn" data-sku-sort data-sort-key="group">Group ${arrow("group")}</button></th><th><button class="sort-btn" data-sku-sort data-sort-key="revenue">DT ${arrow("revenue")}</button></th><th><button class="sort-btn" data-sku-sort data-sort-key="volume">Volume ${arrow("volume")}</button></th><th><button class="sort-btn" data-sku-sort data-sort-key="asp">ASP ${arrow("asp")}</button></th><th>Kênh</th></tr></thead><tbody>${body}</tbody></table></div>`;
    }
    ...
    function renderSkuTrendTable(rows, prevMap, direction) {
      ...
      const body = trendRows.map((row, idx) => {
        const prev = prevMap.get(row.sku) || {revenue:0, volume:0, channelRevenue:{}};
        const img = imageSrc(row);
        return `<tr><td class="rank">${idx+1}.</td><td>${img ? `<img class="sku-thumb" src="${escapeAttr(img)}" alt="${escapeAttr(row.product)}" loading="lazy" />` : ""}</td><td><span class="sku-code">${escapeHtml(row.sku)}</span><div class="subtle">${escapeHtml(row.product)}</div></td><td>${escapeHtml(row.group)}</td><td>${formatCurrency(row.revenue)}</td><td>${deltaHtml(row.deltaRevenue)}</td><td>${channelBreakdownHtml(row, prev)}</td></tr>`;
      }).join("");
